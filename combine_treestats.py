...
parser = OptionParser()
parser.add_option('-i', help='File listing eaching simulation tree_stat files for the different models', )
parser.add_option('-o', help='Output file where all the tree stats for each generation')
		...
		#with open(i, 'r') as f:
			for line in f:
				if line.startswith('#'):
					...
					val = line.strip(')\n').split('(')[1].split(',')
					dest.write('%s\t%s\tTMRCA\t%s\t%s\t%s\t%s\t%s\n' % (gen, model, val[0], val[1], val[2], val[3], val[4] ))
...
param_file = open("model_parameters.txt", 'w')
